...
/*
    Dual Homography
*/
Vec3b bi_linear_interpolation(Mat p, Mat img){
    ...
    {
        /* code */
     pixel = (x - x_floor)*(y - y_floor)*img.at<Vec3b>(y_floor+1,x_floor+1) + (x_floor + 1 - x)*(y_floor + 1 - y)*img.at<Vec3b>(y_floor,x_floor) + (x - x_floor)*(y_floor + 1 - y)*img.at<Vec3b>(y_floor,x_floor+1) + (x_floor + 1 - x)*(y - y_floor)*img.at<Vec3b>(y_floor+1,x_floor);
...
Mat dual_stitching_bg(match_seg mid, match_seg part_A, match_seg part_B, Mat A, Mat B, Mat img_object, int number, Mat &sqrtm_A, Mat &sqrtm_B, int counter_, std::vector<DMatch> & good_matches, Mat& centers) {

    Mat Hl_A = Mat::eye(3,3,CV_64FC1);
    Mat Hl_B = Mat::eye(3,3,CV_64FC1);
    ...
    Mat Hr_A = Mat::eye(3,3,CV_64FC1);
    Mat Hr_B = Mat::eye(3,3,CV_64FC1);

    for (int i = 0; i < 4 - counter_; ++i)
    {
        Hr_A = Hr_A*sqrtm_A.inv();
    }
    for (int i = 0; i < 4 - counter_; ++i)
    {
        Hr_B = Hr_B*sqrtm_B.inv();
    }
